      ...
      <section className="bg-transparent py-20 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-16 reveal-element">
            <div className="flex flex-wrap justify-center gap-1.5 p-1.5 glass-card border border-gray-200 rounded-2xl backdrop-blur-sm">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-5 py-3 rounded-xl text-xs sm:text-sm font-bold tracking-wide uppercase transition-all duration-300 ${
                    activeTab === tab.id
            ...
            {activeTab === 'web' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {webMockups.map((item, index) => (
                  <div 
                    key={item.title} 
                    className="flex flex-col glass-card border border-gray-200 rounded-2xl overflow-hidden shadow-xl hover:border-lavender/30 transition-all duration-300 reveal-element-scale"
            ...
            {activeTab === 'graphics' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {graphicsMockups.map((item, index) => (
                  <div 
                    key={item.title} 
                    className="group glass-card border border-gray-200 rounded-2xl overflow-hidden shadow-lg hover:border-lavender/30 transition-all duration-300 hover:-translate-y-1 reveal-element-scale"
                    style={{ transitionDelay: `${index * 50}ms` }}
                  >
